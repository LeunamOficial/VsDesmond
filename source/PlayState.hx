...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'fiesta':
				{
						defaultCamZoom = 0.75;
						curStage = 'fiesta';
						var bg:FlxSprite = new FlxSprite(-480, -270).loadGraphic(Paths.image('Desmond/fondo1'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var Personajes1:FlxSprite = new FlxSprite(-500, -240);
						Personajes1.antialiasing = true;
						Personajes1.frames = Paths.getSparrowAtlas('Desmond/Personajes1');
						Personajes1.animation.addByPrefix('idle', 'Personajes1', 24);
						Personajes1.animation.play('idle');
						add(Personajes1);
				}				
			case 'schoolEvil':
			{
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | 1.5.4 " + MainMenuState.kadeEngineVer : ""), 16);
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
...

			{
				...
				case 1:
					...
					ready.screenCenter();
					add(ready);
					FlxTween.tween(ready, {y: ready.y += 40, alpha: 0}, Conductor.crochet / 1000, {
				...
				case 2:
					...
					set.screenCenter();
					add(set);
					FlxTween.tween(set, {y: set.y += 40, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							set.destroy();
				...
				case 3:
					...
					go.screenCenter();
					add(go);
					FlxTween.tween(go, {y: go.y += 40, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							go.destroy();
